...
export default function PteMockTest1Page() {
    ...
    useEffect(() => {
        ...
        pteLongReadingTestData.questions.forEach(q => {
            if (q.type === 'fill-in-the-blanks-rw' || q.type === 'fill-in-the-blanks-r') {
                total += q.correctWords?.length || q.blanks?.length || 0;
            } else if (q.type === 're-order-paragraphs') {
                total += 1; // 1 point for the whole question
            }
        });
    ...
    const handleSubmit = () => {
        ...
        pteLongReadingTestData.questions.forEach(q => {
            ...
            if (q.type === 'fill-in-the-blanks-rw') {
                ...
                if (JSON.stringify(userAnswer) === JSON.stringify(q.correctOrder)) {
                    currentScore++;
                }
...
const FillInTheBlanksRComponent = ({ question, onAnswerChange, answers, isSubmitted }: { question: FillInTheBlanksReading, onAnswerChange: Function, answers: (string | null)[], isSubmitted: boolean }) => {
    const initialWordBank = useMemo(() => shuffle([...question.wordBank]), [question.wordBank]);
    const [wordBank, setWordBank] = useState<string[]>([]);
    
    useEffect(() => {
        // Only set initial state once, or when retrying
